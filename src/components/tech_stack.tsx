...
export default function TechStack() {
  ...
  return (
    <section className="flex-col justify-center w-full py-7 bg-gruvbox_gray-hard border-t-2 border-b-2 border-gray-400">
      <section id="projects" className="pl-50  flex justify-start items-start">
      ...
      <div className="flex justify-center">
        <div className="w-7/12 text-gruvbox_yellow">
          <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
            {categories.map((cat) => (
              <div
                key={cat.title}
                className="bg-gruvbox_gray-medium w-11/12 flex-col justify-center py-3 px-6   rounded-xl shadow-lg border md:h-60"
              >
                <h3 className="text-xl font-semibold pb-4">{cat.title}</h3>
                <div className="flex justify-between items-center flex-wrap gap-4">
                  {cat.icons.map(({ name, Icon, color }, idx) => (
                    <div className="flex-col justify-center items-center w-fit text-center">
                      <Icon
                        key={idx}
                        size={80}
                        style={{ color }}
